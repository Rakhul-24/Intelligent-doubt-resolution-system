...
const MeetingRoomPage = () => {
  ...
  const handleConfirmSlot = async () => {
    ...
    try {
      const response = await slotAPI.confirmSlot(slot._id);
      const nextSlot = response.data.slot || slot;
      setSlot((previous) => ({ ...previous, ...nextSlot }));
      setSuccessMessage('Attendance confirmed. You can join the meeting now.');
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (requestError) {
      setError(requestError.response?.data?.error || 'Failed to confirm the online meeting.');
    } finally {
      setConfirming(false);
    }
  };
  ...
  const needsConfirmation =
    access?.isStudentParticipant &&
    slot?.status === 'Pending Student Confirmation' &&
    !completed;
        ...
        <div className="support-header support-header-row" style={{ marginBottom: '1.5rem' }}>
          <div>
            ...
            <p className="dashboard-subtitle">
              Join the scheduled online doubt-resolution meeting directly from the student portal.
            </p>
          </div>
          <button
            type="button"
            className="support-secondary-btn"
            onClick={() => navigate('/doubts/staff?tab=slots')}
          >
            Back to My Online Meetings
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="support-stack">
              ...
              >
                <div>
                  ...
                  <p className="support-muted" style={{ margin: 0 }}>
                    Powered by {getMeetingProviderLabel(slot)} for {user?.name || 'student'}
                  </p>
                </div>
              ...
              <div className="support-list-footer" style={{ alignItems: 'flex-start', gap: '1rem' }}>
                <div style={{ display: 'grid', gap: '0.35rem' }}>
                  <span className="support-muted">
                    Teacher: {slot.staffId?.name || 'Staff member'}
                    {slot.staffId?.subject ? ` - ${slot.staffId.subject}` : ''}
                  </span>
                  <span className="support-muted">
                    Students: {attendeeNames.length ? attendeeNames.join(', ') : 'No students added yet'}
                  </span>
                  {completed && (
            ...
            {completed ? (
              ...
              <div className="island-card support-empty-card">
                This meeting room is only available to the assigned teacher and participating
                students.
              </div>
            )}
          </div>
